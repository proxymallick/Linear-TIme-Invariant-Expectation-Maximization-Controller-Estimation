...
def write_csv(data):
    with open('/home/prakash/gps/python/gps/datapoints_dynamics.csv', 'a') as outfile:
...
class GMM_em(object):
    ...
    def inference(self, pts):
        ...
        return mu0, Phi, m, n0
    def inference_rewards(self, pts):
        """
        Evaluate dynamics prior.
        Args:
            pts: A N x D array of points.
    ...
    def update(self, data, K, max_iterations=100):
        ...
        """         
        d=Dataset(data)
        print "The data are of shapes ---x =",d.x.shape,d.y.shape
        ker = gp.kern.Matern52(8,ARD=True) + gp.kern.White(8)
        m=gp.models.GPRegression(d.x,d.y,ker)
        m.optimize(messages=True,max_f_eval = 1000)
        m.constrain_positive('')
        X=np.linspace(3,10,N*d.x.shape[1]).reshape(N,d.x.shape[1])
        mu_new1, cov_new1= m.predict(X,full_cov=True)
        print m.log_likelihood() 
        """
        
        #   kernel = 1.0 * RBF([1.0])
        #    gpc_rbf_isotropic = GaussianProcessClassifier(kernel=kernel).fit(d.x, d.y)
        #   g = GaussianProcess()
        #g_log_likelihood= g.fit(d)
