...
def higher_dimensions_dynamics_learn_time_varying(X,T):
    
    gmm = GMM(X,k=40, eps=0.0001)
    if np.isnan(X).any():
    ...
    m = X_clipped.shape[0]
    dwts = (1.0 / X_clipped.shape[0]) * np.ones(X_clipped.shape[0])
    n0 = m - 2 - mu_moment.shape[0]
    # Normalize.
    m = float(m) / X_clipped.shape[0]
    n0 = float(n0) / X_clipped.shape[0]
    ...
    ## Time Varying  case             ######
    ##########                 ###################                         ##################
    Fm_total = np.zeros((T, dX, dX+dU))
    fv_total = np.zeros((T, dX))
    dyn_covar_total = np.zeros((T, dX, dX))
    for t in range(T - 1):
        Ys = np.c_[X_clipped[:, t, :6], X_clipped[:, t, 6:8], X_clipped[:, t+1, :6]]
        Fm, fv, dyn_covar = gauss_fit_joint_prior(Ys,mu_moment, sigma_moment, m, n0, dwts, dX+dU, dX, sig_reg)
        Fm_total[t,:,:]=Fm
        fv_total[t,:]=fv
        dyn_covar_total[t,:,:]=dyn_covar
        #print sigma_moment
        np.linalg.cholesky(dyn_covar_total[t,:,:])
    

    return Fm_total,fv_total,dyn_covar_total,params.log_likelihoods



def reward_learn_time_varying_case(X):

    gmm = GMM(X,k=1, eps=0.0001)
    params = gmm.fit_EM(X, max_iters= 200)
    X_clipped = X.reshape((5,T,9))
    dX=6
    dU=2
    dR=1
    k=1
    ...
    ## Time Varying  case             ######
    ##########                 ###################                         ##################
    Fm_total = np.zeros((T, 1, dX+dU))
    fv_total = np.zeros((T, 1))
    dyn_covar_total = np.zeros((T, 1, 1))
    for t in range(T-1):
        Ys = np.c_[X_clipped[:, t, :6], X_clipped[:, t, 6:8], X_clipped[:, t, -1]]
        Fm, fv, dyn_covar = gauss_fit_joint_prior(Ys,mu_moment, sigma_moment, m, n0, dwts, dX+dU, dX, sig_reg)
...
def higher_dimensions_dynamics_learn_from_gps_gmm_em(X):
    ...
    for t in range(T - 1):
        Ys = np.c_[X_clipped[:, t, :6], X_clipped[:, t, 6:8], X_clipped[:, t+1, :6]]
        mu0, Phi, m, n0 = gmm_obj.inference(Ys)
        mu_moment = mu0
        sigma_moment = Phi
    ...
    return Fm_total,fv_total,dyn_covar_total


def reward_learn_from_gps_gmm_em(X):
...
def main_call(T,X,K,k,sig):
    ...
    A_dyn,B_dyn,sigma_dyn,log_likelihood_dynamics_fitting = higher_dimensions_dynamics_learn_time_varying(X,T)
    #A_dyn,B_dyn,sigma_dyn = higher_dimensions_dynamics_learn_from_gps_gmm_em(X)
    #print A_dyn
    
    X_clipped_1 = X[  X.shape[0]-1-5*(T):-1 ,:].reshape((5,T,14))
    X_clipped=X_clipped_1.reshape((5*T,14))
    beta=1  
    Q_dim=6
    gamma=1
    q_dim=2
    ...
    reward=np.zeros((5*T,1))
    R_t=np.zeros((T,1))
    target_state= np.array([ 20.0, 3.0, 0.0, 0.0,  0.0, 0.0]).reshape(1,6)
    ##################################################################################################
    #### Simulate the equations to produce the reward values (beta* ||(x-x)||^2 + ||(u-u)||^2)
    ###################################################################################################
    for iter in range (5*T):
        reward[iter]= gamma *  ( np.dot((X_clipped[iter,:6]-target_state),np.dot(Q,np.transpose((X_clipped[iter,:6]-target_state)))) \
                               + np.dot(X_clipped[iter,6:8],np.dot(q, np.transpose(X_clipped[iter,6:8])) ) )
    ...
    sum_discounted_reward =  (beta*np.exp(-beta*R_t)).reshape(5*T,1)
    #X_U_REW=X[  X.shape[0]-1-5*(T):-1 ,:].reshape((5,T,14))
    reshaped_reward= sum_discounted_reward.reshape((5,T,1))
    print reshaped_reward.shape
    reward_with_data_points = (np.hstack( (X_clipped_1[:,:,:8].reshape((5*T,8))[:,:8],reshaped_reward[:,:,:].reshape((5*T,1))[:,:1] )))
    print reward_with_data_points.shape
    
    A_rew,B_rew,sig_reward,log_likelihood_reward_fitting = reward_learn_time_varying_case(reward_with_data_points)
    #A_rew,B_rew,sig_reward= reward_learn_from_gps_gmm_em(reward_with_data_points)
    ...
    if check==True:
        ...
        EM_kalman_time_varying_case(T,K1,k1,sig1,X,reward_with_data_points,A_dyn,B_dyn,sigma_dyn,A_rew,B_rew,sig_reward,target_state) 
